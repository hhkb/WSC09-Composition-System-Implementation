 ...
 * Original Filename: org.dgpf.machine.ll.instructions.arith.Mod.java
...
/**
 * This instruction modulo divides two numbers.
 * 
 * @author Thomas Weise
 */
public class Mod extends ArithInstruction {
   ...
   * the operation text
   */
  static final char[] OP_TXT = " mod ".toCharArray(); //$NON-NLS-1$
   ...
   */
  public Mod(final InstructionSet isb) {
    super(isb);
  }
  ...
  public void execute(final VirtualMachine<LGPMemory, LGPProgram> vm,
    ...
    int r;
    r = EIndirection.read(vm, param3);
    if (r == 0)
      vm.setError();
    else if (r == 1)
      EIndirection.write(vm, param1, 0);
    else
      EIndirection.write(vm, param1, EIndirection.read(vm, param2) % r);
  }
  ...
  protected boolean isUseless(final EIndirection indir1, final int index1,
    ...
    ((indir3 == EIndirection.CONSTANT) && (index3 == 0)));
  }
