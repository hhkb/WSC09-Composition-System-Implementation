 ...
 * Original Filename: test.org.sigoa.dmc.SteadyStateEA.java
...
public class SteadyStateEA {
  ...
  public static void main(String[] args) {
    ...
    pp = Mathematics.nextPrime((6 * 1024) + 1);// ((4 * 1024) + 1);

    System.out.println("Population-Size: " + pp); //$NON-NLS-1$

    opt = new ElitistEA<byte[], ClassifierSystem>(pp, 0.4d, 0.4d);

    opt
        .addNonPrevailedPipe(new IndividualPrinterPipe<byte[], ClassifierSystem>(
            new FileTextWriterProvider(new File(s_bd + "c"), //$NON-NLS-1$
                "c"), false)); //$NON-NLS-1$
    opt
        .addNonPrevailedPipe(new ObjectivePrinterPipe<byte[], ClassifierSystem>(
