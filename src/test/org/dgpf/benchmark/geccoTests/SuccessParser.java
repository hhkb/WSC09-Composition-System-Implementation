...
public class SuccessParser {
   ...
   * the parse dir
   */
  private static String PARSE_DIR = "d:\\temp\\newModel\\ruggednessTest2"; //$NON-NLS-1$

  /**
   * the output dir
   */
  private static String OUTPUT_DIR = "d:\\temp\\newModel\\ruggednessTest2"; //$NON-NLS-1$
  ...
  private static final boolean isNumber(final double d) {
    return (Mathematics.isNumber(d) && (d > (0.9d * Long.MIN_VALUE)) && (d < (0.9d * Long.MAX_VALUE)));
  }
  ...
  public static void main(final String[] args) {
    ...
    if ((v = l.size()) > 0) {
      ...
      for (i = 0; i < v; i++) {
        ...
        bsSP = (isNumber((d[CNT_IDX] - d[FAILED_S_CNT_IDX]) / d[CNT_IDX]));
        ...
        bpSP = (isNumber((d[CNT_IDX] - d[FAILED_P_CNT_IDX]) / d[CNT_IDX]));
        ...
        if (isNumber((d[CNT_IDX] - d[FAILED_S_CNT_IDX]) / d[CNT_IDX]))
        ...
        if (isNumber((d[CNT_IDX] - d[FAILED_P_CNT_IDX]) / d[CNT_IDX]))
