...
public class StdDevObjective
   ...
   *          the steps required
   */
  public StdDevObjective(final int requiredSteps) {
  ...
  public void endEvaluation(
    ...
    if (n.m_cs_used) {
      ...
      ss = 0l;
      for (i = (c - 1); i >= 0; i--) {
        v = ((ICSVM) (n.getVirtualMachine(i))).getCSTimes();
        s += v;
        ss += (v * v);
      }
      ...
      if (s > 0) {
        avg = (s / ((double) c));
        stddev = /* Math.sqrt( */((ss / ((double) c))) - (avg * avg)/* ) */;

        state.setObjectiveValue(stddev);
      }
    }
